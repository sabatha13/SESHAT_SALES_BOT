...
export async function POST(req: NextRequest) {
  ...
  const { bookId, pageNumber, text, color, note } = await req.json();
  if (!bookId || !pageNumber || !text) return NextResponse.json({ error: 'Paramètres manquants' }, { status: 400 });
  ...
  const { data } = await supabase
    .from('highlights')
    .insert({ user_id: profile.id, book_id: bookId, page_number: pageNumber, text, color: color || 'yellow', note })
    .select()
    .single();
...
export async function DELETE(req: NextRequest) {
  ...
  await supabase.from('highlights').delete().eq('id', id).eq('user_id', profile.id);
